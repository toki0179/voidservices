...
def run(playwright: Playwright) -> None:
    import logging
    # Wrap the entire function body to catch and log any internal error that causes exit
    try:
        ...
        if page.locator("text=You are being rate limited").is_visible():
            ...
            context.close()
            browser.close()
            print("LOG:Process exiting because of rate limit")
            return
        ...
        context.close()
        browser.close()
        print("LOG:Process finished normally (successful registration)")
    except Exception as e:
        print(f"LOG:Process quit due to exception inside run(): {e}")
        raise  # re-raise to let outer handler also log

# Main execution wrapper – log any exit reason from the whole script
if __name__ == "__main__":
    try:
        with Stealth().use_sync(sync_playwright()) as playwright:
            run(playwright)
    except KeyboardInterrupt:
        print("LOG:Process quit due to KeyboardInterrupt (user interrupted)")
    except Exception as e:
        print(f"LOG:Process quit due to unhandled exception: {e}")
        raise
    else:
        print("LOG:Process exited normally without errors")